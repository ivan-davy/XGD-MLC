...
def mlLoadSets():
    sp_sets = {}
    subdirs = [f.path for f in os.scandir(f'{"../"}{config.src_fileset_location}') if f.is_dir()]
        ...
        for root, dirs, files in os.walk(subdir):
            for file in files:
                if file.endswith('.sps'):
                    spectra.append(file)
        sp_sets[subdir.rsplit('/', 1)[-1]] = spectra
    return sp_sets
...
def mlGetFeatures(ml_set, feature_type, bins_per_sect=config.ml_bin_clf_bins_per_section,
    ...
    if feature_type == 'linfit':
        ...
        for spectrum in ml_set:
            ...
            if show_progress:
                print('\r', counter, '/', len(ml_sets), spectrum.location, end='')
        if show_progress:
            print('\n')
        return set_a, set_b
...
def mlCreateModel(sp_sets, feature_type, method,
    ...
    try:
        ...
        if feature_type == 'linfit':
            source_ml_set_a, source_ml_set_b = mlGetFeatures(sp_sets,
                                                             feature_type,
...
def mlClassifier(test_spectrum_set, out, show, **user_args):
    ...
    try:
        ...
        sp_sets = mlLoadSets()
        ml_clf_model = mlCreateModel(sp_sets,
                                     user_args["Feature"],
                                     user_args["Method"],
