...
def loadSpectrumData(sps_filename):
    ...
    spectrum = gSpectrum(sps_filename, num_of_bins, real_time_int, live_time_int, calibration, bin_data)
    if calibration[0] == 0:
...
def printConfusionMatrix(sigs, bkgs, bkg_sp_reference, method, tolerance, los):
    res, tp, fp, fn, tn = None, 0, 0, 0, 0
    for spectrum in sigs:
