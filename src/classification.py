...
def classification(**user_parsed_args):
    with Profile() as pr:
        ...
        if user_parsed_args['MethodBinary'] == 'sigma':
            ...
            mlBinaryClassifier(test_spectrum_set, out, show=False, **user_parsed_args)
        else:
            print('\nWrong method. Try again!')
    '''
    print('Preparing sets...')
    sigs = prepareSet(mlLoadSets()[0])
    bkgs = prepareSet(mlLoadSets()[1])
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'pearson', tolerance=1, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'pearson', tolerance=3, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'pearson', tolerance=5, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'pearson', tolerance=10, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'chi2square', tolerance=1, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'chi2square', tolerance=3, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'chi2square', tolerance=5, los=0.05)
    printConfusionMatrix(sigs, bkgs, bkg_spectrum, 'chi2square', tolerance=10, los=0.05)
    '''
...
if __name__ == '__main__':
    parser = ArgumentParser(description='Spectra classification ML algorithms')
    ...
    parser.add_argument('-S', '--SigSet',
                        help='location of signal spectra set',
                        default=config.sig_fileset_location,
                        type=str)
    ...
    parser.add_argument('-o', '--Output',
                        ...
                        default=config.report_location,
                        type=str)
    parser.add_argument('-sc', '--Scale',
                        help='perform data pre-processing (boolean)',
    ...
    parser.add_argument('-f', '--Feature',
                        ...
                        type=str)
    parser.add_argument('-fb', '--FeatureBinary',
